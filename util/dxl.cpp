...
bool dxlInit(dxl_t *p_packet, uint8_t protocol_ver)
{
  ...
  p_packet->rx_timeout = 50000000;
  p_packet->tx_timeout = 50000000;
...
void dxlSetDirPin(dxl_t *p_packet, int8_t dir_pin)
{
  ...
  digitalWrite(dir_pin, DXL_DIR_PIN_TX);
}

void dxlSetSerial(dxl_t *p_packet, Duplex *p_serial)
{
...
{
  return (uint32_t) p_packet->p_serial->available();
}
...
dxl_error_t dxlRxPacket2_0(dxl_t *p_packet, uint8_t data_in)
{
  ...
  uint32_t prevTime = p_packet->prev_time;
  uint32_t timeOut = p_packet->rx_timeout;
